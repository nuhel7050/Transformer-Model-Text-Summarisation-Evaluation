"""T5-Base summarizer with hierarchical chunking for long documents."""
...
# Model constants
MODEL_NAME = "t5-base"
MAX_TOKENS = 507  # Reserve ~5 tokens for the 'summarize:' prompt prefix
CHUNK_OVERLAP = 50


class T5Summarizer(BaseSummarizer):
    """Summarizer using the T5-Base model.

    For documents exceeding the 512-token context window, applies
    ...
    def _summarize_batch(self, batch):
        ...
        Returns:
            Dictionary with 't5_summary' key containing list of summaries.
        """
        summaries = [self.summarize_single(text) for text in batch["text"]]
        return {"t5_summary": summaries}
